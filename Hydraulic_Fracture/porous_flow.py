    ...
    def addToIndependentVector(self):
        ...
        def leakoff(): #Adiciona o leakoff ao termo fonte como uma condição de Neumann
            ...
            for facet in sup.facets:
                for outerFace in facet.outerFaces:
                    self.independent[outerFace.vertex.handle] += 1 * np.linalg.norm(outerFace.area.getCoordinates()) #leak_sup_old[i] * np.linalg.norm(outerFace.area.getCoordinates())
                    i = i + 1
            j = 0
            for facet in inf.facets:
                for outerFace in facet.outerFaces:
                    self.independent[outerFace.vertex.handle] += 1 * np.linalg.norm(outerFace.area.getCoordinates()) #leak_inf_old[j] * np.linalg.norm(outerFace.area.getCoordinates())
                    j = j + 1
