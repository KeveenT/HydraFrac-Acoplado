...
# diff_pressure = np.mean(FINAL_PRESSURE) - np.mean(PENULT_PRESSURE)
# print("A diferença entre a pressão média do meio nos últimos instantes de tempo é em Pascal:", diff_pressure)

np.savetxt("final_pressure_field.csv", FINAL_PRESSURE, delimiter=",")

CONTORNO = plt.tricontourf(X, Y, FINAL_PRESSURE, 100)
COLOR_BAR = plt.colorbar()
COLOR_BAR.set_label('Pressão')
plt.savefig('contorno_press', dpi=1200)
plt.figure()
...
np.savetxt("pn.csv", Pn, delimiter=",")
...
plt.legend()
plt.grid()
plt.savefig('p_elements.pdf', dpi=1200)
plt.figure()
...
plt.grid()
plt.savefig('p_tip.pdf', dpi=1200)
plt.figure()
